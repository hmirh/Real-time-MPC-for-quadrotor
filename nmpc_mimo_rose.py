...
csvfile = open('experiment_rose_mpc.csv','w',encoding="utf-8")
...
#----------------------------------------------------------------
    ...
    def tvp_fun(t_now):
        angle = math.pi / 5
        sx = 1 * math.cos(2 * angle * t_now) * math.cos(angle * t_now)
        sy = 1 * math.cos(2 * angle * t_now) * math.sin(angle * t_now)
...
x00 = np.array([1, 0, 0, 0]).reshape(-1,1)
